...
function chooseTurnRaw(s, turnN) {
  ...
  const regionsSecured = [zap>=85,khe>=65,kha>=50].filter(Boolean).length;
  const territorySecured = don>=100 && luh>=100 && regionsSecured>=2;
  const betrayals = s.betrayal_count ?? 0;

  // === ПОСЛЕ ВТОРОГО ВЕРОЛОМСТВА: мир мёртв — только война ===
  // «Один раз обманули — виноват Киев. Два раза — виноват я. Хватит. Только сила.»
  if (betrayals >= 2 && !territorySecured) {
    if (eco<36) return { action:"decree_fast", input:"Перевод экономики на военные рельсы — финансирование решающего наступления.", label:"ВОЕННАЯ ЭКОНОМИКА", think:"Киев предал дважды. Мира не будет. Но сначала не дам казне рухнуть." };
    if (init>=55 && mil>=72) return { action:"military", input:`Решительное наступление — закрыть все территориальные цели силой (${milTargets()}). Переговоры окончены.`, label:`ВОЙНА ДО КОНЦА (${milTargets()})`, think:"Дважды обманутый — сам виноват. Сворачиваю дипломатию, дожимаю военным путём." };
    return { action:"regroup", input:null, label:"ПЕРЕГРУППИРОВКА (война)", think:"Накоплю инициативу для следующего удара — теперь только фронт." };
  }

  // === ДОЖАТЬ ПОБЕДУ (приоритет выше всего, кроме реального коллапса) ===
  // Если до принуждения к миру остался военный удар (Донбасс закрыт, 1 регион есть,
  // мир≥42), и экономика ещё не в коллапсе — бьём, не отвлекаясь на латание тыла.
  const oneStrikeFromCombined = don>=100 && luh>=100 && regionsSecured>=1 && peace>=42;
  if (oneStrikeFromCombined && eco>32 && mil>=72) {
    return { action:"military", input:`Решающий удар — закрыть второй регион (${milTargets()}) и зафиксировать принуждение к миру с позиции силы.`, label:`ДОЖАТЬ COMBINED (${milTargets()})`, think:"Донбасс взят, мир набран, один регион есть — финальным ударом закрываю второй и получаю принуждение к миру. Экономику латать некогда." };
  }

  // === АВАРИЙНЫЙ ТЫЛ ===
  if (eco<40) return { action:"decree_fast", input:"Экстренная экономическая поддержка — резервы на рынок и оборонку.", label:"ПОДДЕРЖКА ЭКОНОМИКИ", think:"Экономика просела — без неё всё рухнет, латаю немедленно." };

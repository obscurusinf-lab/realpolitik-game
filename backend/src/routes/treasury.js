 ...
 *   - ⚡40 инициативы, −15 казны, кулдаун 4 хода (stats.emergency_stimulus_last_turn)
 *   - Эффект: economy +10 немедленно, инфляция +5 сразу
 *   - "Похмелье": 3 месяца подряд −1 к economy (perk_stimulus_hangover_turns, тикает в
 *     /turns/end-month) — цена платится ПОСЛЕ, не разово наперёд, как настоящий адреналин
 */
...
const EMERGENCY_STIMULUS_INFLATION_HIT = 5;
const EMERGENCY_STIMULUS_COOLDOWN_TURNS = 4;   // ходов между применениями
const EMERGENCY_STIMULUS_HANGOVER_TURNS = 3;   // месяцев отложенной цены (-1 economy/мес)
...
async function registerTreasuryRoutes(fastify, { db, verifyToken }) {
  ...
  fastify.post("/games/:gameId/treasury/emergency-stimulus", async (request, reply) => {
    ...
    try {
      ...
      const newsText = `Экстренные вливания в экономику: правительство направило резервы на прямую поддержку — экономика ${economyBefore}→${newStats.economy}. Эффект придётся отработать: инфляционное давление выросло сразу, а ближайшие ${EMERGENCY_STIMULUS_HANGOVER_TURNS} месяца потребуется время на восстановление после интервенции.`;

      await client.query(
        `INSERT INTO newsfeed_items (game_id, turn_n, item_type, source, text, reactions) VALUES ($1,$2,'news',$3,$4,$5)`,
        [gameId, game.current_turn + 1, "Минфин", newsText, JSON.stringify([{ stat_delta: { economy: EMERGENCY_STIMULUS_ECONOMY_BOOST, inflation: EMERGENCY_STIMULUS_INFLATION_HIT } }])]
      );

      await client.query("COMMIT");
      return reply.send({ stats: newStats, economyBefore, economyAfter: newStats.economy });
    } catch (err) {
      await client.query("ROLLBACK");
      fastify.log.error(err);

...
// Осведомлённость о Башнях Кремля (Петя, 2026-07-10: "а в этой системе помощи учитываются башни
// Кремля?" — до этого фикса не учитывались вообще, ни в одной ветке). Ниже 60 у башни уже активна
// лестница дебаффов (FACTION_DEBUFF_LADDER в rules-engine.js) — фоновая просадка статов, которая
// раньше была полностью невидима советнику. Показывает только САМУЮ недовольную башню — не нужно
// перечислять все 4, если проблема реально в одной.
const FACTION_LABELS_RU = {
  faction_siloviki: "силовики", faction_tehnokraty: "технократы",
  faction_oligarhi: "олигархи", faction_konservatory: "консерваторы",
};
function describeFactionPressure(stats) {
  const worst = FACTION_KEYS
    .map(k => ({ key: k, value: typeof stats[k] === "number" ? stats[k] : 65 }))
    .filter(f => f.value < 60)
...
function buildAdvisorsUserMessage({ countryName, playerName, gameDate, turnNumber, stats, relations, policies, recentHistory, playerDraft, actionMode, optimalMove, statHistory, language, advisorId, guided }) {
  // Режим "Обучение" (guided, 2026-07-20) — Петя: "министр финансов рассказывает про казну
  // простыми словами" — не отдельная персона, а инструкция ВСЕМ существующим персонажам объяснять
  // проще, для игрока без бэкграунда в политике/экономике/военном деле. Персонаж (голос, манера)
  // сохраняется — меняется только словарь: без терминов и цифр без пояснения, что они значат.
  const guidedSection = guided
    ? `═══════════════════════════════════════════
РЕЖИМ "ОБУЧЕНИЕ" — ОБЪЯСНЯЙТЕ ПРОСТО
═══════════════════════════════════════════
Президент новичок, первый раз в теме. Оставайтесь СВОИМ персонажем (голос, манера, характер —
без изменений), но:
- Не используйте термины без объяснения тут же простыми словами (не "инфляция 8,2%", а "цены
  растут — люди могут купить меньше на те же деньги").
- Каждое число сопровождайте пояснением, что оно значит на практике, а не просто называйте его.
- Короче и конкретнее — 2-3 предложения, не 3-4.
- Не бойтесь начать с базового объяснения "что вообще происходит в моей сфере", если это уместно —
  президент может не понимать матчасть, а не только не знать, что делать.
`
    : "";
...
КРИТИЧНО: если советник называет размер казны в триллионах рублей — он ОБЯЗАН использовать ТОЧНО
то число, что указано выше в строке "Казна: X трлн ₽". ЗАПРЕЩЕНО придумывать другую цифру
("100 триллионов" и т.п.) — это реальный игровой показатель, а не риторический оборот.

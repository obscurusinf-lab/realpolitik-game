  ...
  return (
    <div>
      <div style={{ marginBottom: 10 }}>
        <div className="mono-font" style={{ fontSize: 9, color: "#5a6070", letterSpacing: "0.1em", marginBottom: 6 }}>МАСШТАБ РЕШЕНИЯ:</div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div style={{ background: "#14181f", border: "1px solid #3a4156", borderTop: "3px solid #9c8347", borderRadius: 6, maxWidth: 600, width: "100%", maxHeight: "92vh", overflow: "auto", boxShadow: "0 30px 80px rgba(0,0,0,0.8)" }}>
        ...
        <div style={{ padding: "22px 22px 28px" }}>
          ...
          {/* Настойчивое напоминание прочитать Ликбез — этот брифинг даёт только базовый обзор,
              а Ликбез систематизирован по разделам и содержит рекомендуемый стиль игры,
              выведенный из сыгранных партий. Скрыт в hardcore-режиме — там сознательно нет подсказок. */}
          {assistMode !== "hardcore" && onOpenWiki && (
            <div style={{ background: "#241a10", border: "1px solid #9c8347", borderRadius: 4, padding: "12px 14px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div className="doc-font" style={{ fontSize: 12.5, color: "#e0c878", lineHeight: 1.5 }}>
                📖 Этот брифинг — только обзор. Прежде чем действовать, откройте <b>Ликбез</b> — там вся механика по разделам и разбор того, что реально работает.
              </div>
              <button
                onClick={onOpenWiki}
          ...
          <BriefSection id="howto" label="📖 КАК ИГРАТЬ" color="#5a6070">
            <div style={{ display: "grid", gap: 10 }}>
              {[
                ...
                ["7", "Прочитайте Ликбез", "Обязательно — кнопка «📖 ЛИКБЕЗ» в шапке. Там систематизированы все механики по разделам и отдельно разобран рекомендуемый стиль игры, выведенный из уже сыгранных партий."],
              ].map(([n, title, desc]) => (
                  ...
                  <div className="doc-font" style={{ fontSize: 13, color: "#ece7d8", marginBottom: 10 }}>
                    {policy.cancel_penalty && Object.keys(policy.cancel_penalty).length > 0
                      ...
                      : (() => {
                          ...
                          const bits = [`стабильность −2`, `рейтинг ${approvalDelta}`];
                          if (economyDelta) bits.push(`экономика ${economyDelta}`);
                          const budgetBits = [];
                          if (income > 0) budgetBits.push(`казна теряет ${income}/мес. дохода`);
                          if (upkeep > 0) budgetBits.push(`казна экономит ${upkeep}/мес. на содержании`);
                          return <>Отмена даст штраф: {bits.join(", ")}.{budgetBits.length ? ` ${budgetBits.join(", ")}.` : ""} Продолжить?</>;
                        })()}
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
...
function buildCardVariants(card, tier, seed = 0) {
  ...
  if (pool) {
    ...
    return picked.map((text, i) => ({ label: `Вариант ${i + 1}`, text }));
  }
            ...
            >
              ...
              {isExpanded && (() => {
                ...
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #2a3040" }} onClick={e => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div className="mono-font" style={{ fontSize: 9, color: "#7a8294", letterSpacing: "0.05em" }}>ВЫБЕРИТЕ ФОРМУЛИРОВКУ</div>
                    {hasMore && (
                      <button
                        onClick={() => { setVariantSeed(s => s + 1); setSelectedVariant(0); setCustomText(""); }}
                        title="Показать другие формулировки"
...
function TreasuryTab({ state, gameId, onRefresh }) {
  ...
  return (
    <div ref={treasuryGridRef} style={{ position: "relative", height: masonryHeight || undefined, minHeight: 200 }}>
      ...
      <WidgetCard {...widgetCardProps("economy")} label="ЭКОНОМИКА — ЧТО ЕЁ ДВИГАЕТ">
      ...
      <WidgetCard {...widgetCardProps("balance")} label="МЕСЯЧНЫЙ БАЛАНС (ПРОГНОЗ)">
      ...
      <WidgetCard {...widgetCardProps("ofz")} label="ОФЗ — ГОСУДАРСТВЕННЫЙ ДОЛГ">
        <div style={{ background: "#14181f", borderRadius: 6, padding: "14px 16px", border: `1px solid ${ofzCount > 0 ? "#5a3a10" : "#2a3040"}` }}>
          ...
          {ofzCount > 0 && (
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: "#c08050", marginBottom: 10 }}>
              ...
              (≈₽{(ofzDebt * T).toFixed(1)} трлн) · Инфляционное давление +{Math.round(ofzCount * 0.3 * 10) / 10}/мес.
            </div>
          )}
          {ofzCount === 0 && (
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            >
              {loading === "issue" ? "Выпуск…" : ofzUsedThisMonth ? "⚠ Выпуск уже использован в этом месяце" : ofzCount >= OFZ_MAX ? "✕ Лимит долга исчерпан" : `📄 Выпустить ОФЗ (+20 казны; далее +0.3 инфляции/мес., пока выпуск активен)`}
            </button>
            <button
              onClick={handleRepay}
            ...
            >
              {loading === "repay" ? "Погашение…" : `💸 Погасить выпуск (−22 казны, премия за досрочный выкуп; далее −0.3 инфляции/мес.)`}
            </button>
          </div>
      ...
      {(() => {
        ...
        return (
          <WidgetCard {...widgetCardProps("keyrate")} label="КЛЮЧЕВАЯ СТАВКА ЦБ">
      ...
      {(() => {
        ...
        return (
          <WidgetCard {...widgetCardProps("corruption")} label="КОРРУПЦИЯ">
            <div style={{ background: "#14181f", border: "1px solid #2a3040", borderRadius: 6, padding: "14px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  ...
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: "#5a6070", marginTop: 3 }}>
                    индекс восприятия коррупции (Transparency International) · <b>выше = лучше</b> (меньше коррупции) · {corrLevel > 65 ? "высокий риск скандалов" : corrLevel > 50 ? "заметная утечка бюджета" : "под контролем"}
                  </div>
                  ...
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: "#3a4050", marginTop: 1 }}>
                    внутренний уровень тяжести: {Math.round(corrLevel)}/100 (выше = хуже — обратно CPI; им и заполнена шкала ниже)
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 14, fontWeight: 700, color: corruptionDrainT > 0 ? "#c05050" : "#5a8a6a" }}>
                    {corruptionDrainT > 0 ? `−₽${corrDrainRubT.toFixed(1)} трлн/мес.` : "нет (уровень ≤ 50)"}
                  </div>
                </div>
              </div>
      ...
      {(() => {
        ...
        return (
          <React.Fragment>
          <WidgetCard {...widgetCardProps("reserves")} label="РЕЗЕРВЫ (ФНБ)">
            <div style={{ background: "#14181f", border: "1px solid #2a3040", borderRadius: 6, padding: "14px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
      ...
      {(() => {
        ...
        return (
          <React.Fragment>
          <WidgetCard {...widgetCardProps("fxregime")} label="КУРСОВАЯ ПОЛИТИКА">
            <div style={{ background: "#14181f", border: "1px solid #2a3040", borderRadius: 6, padding: "14px 16px" }}>
      ...
      {(() => {
        ...
        return (
          <WidgetCard {...widgetCardProps("inflation")} label="ИНФЛЯЦИОННОЕ ДАВЛЕНИЕ">
      ...
      {(() => {
        ...
        return (
          <WidgetCard {...widgetCardProps("oilfx")} label="НЕФТЕДОХОДЫ И ВАЛЮТА">
...
const WIKI_SECTIONS_META = [
  { id: "strategy", icon: "🎯", label: "Стиль игры" },
  { id: "resources", icon: "⚡", label: "Ресурсы" },
  { id: "stats", icon: "📊", label: "Показатели" },
  { id: "kremlin", icon: "★", label: "Башни Кремля" },
  { id: "ukraine", icon: "🇺🇦", label: "Украина" },
  { id: "econ", icon: "🛢", label: "Экономика — детали" },
  { id: "policies", icon: "⚙", label: "Политики" },
  { id: "victory", icon: "🏆", label: "Победа и поражение" },
  { id: "advisors", icon: "🗣", label: "Советники" },
];
...
function WikiTab({ dark = false }) {
  ...
  const WikiSection = ({ id, label, children }) => {
    ...
    return (
      <div ref={el => { sectionRefs.current[id] = el; }} style={{ marginBottom: 10, scrollMarginTop: 12 }}>
        <button onClick={() => toggleSection(id)}
          ...
          <span style={S.h}>{meta?.icon} {label}</span>
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <div className="doc-font" style={{ fontSize: 13.5, color: dark ? "#7a8294" : "#6a6458", marginBottom: 14 }}>Разделы кликабельны — открывайте по одному, не обязательно читать всё подряд. Если времени мало — начните с «Стиль игры».</div>
      ...
      <WikiSection id="strategy" label="РЕКОМЕНДУЕМЫЙ СТИЛЬ ИГРЫ">
        <div className="doc-font" style={{ fontSize: 12, color: dark ? "#8a9aaa" : "#8a8060", marginBottom: 14, fontStyle: "italic" }}>
          Не теория — выводы из 11 сыгранных до конца партий (включая победную). Каждый пункт — реальная причина поражения или затяжки в одной из них.
        </div>
        <div style={{ display: "grid", gap: 10 }}>
          {[
            ["1", "Останавливайтесь ДО, а не реагируйте ПОСЛЕ", "Главный урок победной партии: пауза наступлений включалась заранее — при казне <25, а не когда она уже ушла в минус. Возобновление — только когда накопился буфер >55, не «на грани». Реактивное восстановление (стимул постфактум) почти никогда не разворачивает уже начавшуюся дефицитную спираль — предотвратить дешевле, чем чинить."],
            ["2", "Не более 1-2 наступлений подряд без восстановительного хода", "Несколько атак подряд без передышки/перегруппировки почти гарантированно роняют экономику через военное бремя + дефицит казны одновременно — в нескольких партиях это была прямая причина экономического коллапса."],
            ["3", "Дипломатию нужно укреплять отдельно, не «само подтянется»", "В одной из партий дипломатия рухнула 48→11 и привела к изоляции просто потому, что ей не уделяли внимания, пока решались военные и экономические вопросы. Держите хотя бы фоновый дипломатический трек параллельно с любой другой стратегией."],
            ["4", "Формулируйте указ под нужный эффект явно", "Если цель — поднять рейтинг, пишите указ как социальную/политическую меру, а не заворачивайте её в экономические термины: ИИ-классификатор один раз принял социальный указ за экономический стимул, эффект на рейтинг оказался значительно слабее ожидаемого, и партия чуть не закончилась переворотом."],
            ["5", "Занятая территория требует периодического подкрепления", "Фронт — не «заморозка»: если несколько месяцев подряд не подкреплять взятые территории, ВСУ реально их отбивают обратно (контрнаступление считает вашу текущую армию/мораль, а не факт наличия территории). Нельзя взять регион и забыть о нём на много ходов."],
            ["6", "Инициатива — реальное узкое место при нескольких действиях за месяц", "При пакетировании 2-3 решений в один календарный месяц инициатива кончается быстрее, чем кажется. Не начинайте дорогую военную операцию, если казна уже просела — в паре партий это привело к тому, что армию нечем было прокормить в конце того же месяца."],
            ["7", "Итоговое окно для полной победы узкое, но реальное", "Одновременно нужно: территория 100/100 + 2 из 3 регионов, армия ≥85, мораль/готовность ≥70, тыл (стабильность/рейтинг) ≥52, экономика ≥36, мирный трек ≥40. Это не везение — это дисциплина ресурсов на всей дистанции партии, а не удачный последний рывок."],
          ].map(([n, title, desc]) => (
            <div key={n} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div>
                <div className="doc-font" style={{ fontSize: 13.5, fontWeight: 700, color: dark ? "#ece7d8" : "#2a2620", marginBottom: 2 }}>{title}</div>
                <div className="doc-font" style={{ fontSize: 12.5, color: dark ? "#8a9aaa" : "#6a6458", lineHeight: 1.5 }}>{desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <WikiSection id="resources" label="РЕСУРСЫ">
        <div style={S.p}><span style={S.b}>Инициатива</span> — политическая воля президента. Восстанавливается полностью в начале каждого месяца. Каждое действие тратит её — от 15 за разведку до 80 за стратегическое наступление, точная стоимость указана на карточке категории во вкладке «Башни Кремля». Если инициативы не хватает — действие недоступно.</div>
      ...
      <WikiSection id="stats" label="ПОКАЗАТЕЛИ">
        ...
        <div style={S.p}><span style={S.b}>Армия</span> — боеспособность вооружённых сил. Ниже 30 — поражение (фронт рухнул), а Украина получает шанс отбить Донбасс. Складывается из морального духа, готовности, техники и боевого опыта ветеранов — чем выше опыт войск, тем результативнее наступления. Военные операции двигают территории, но стоят инициативы и денег.</div>
        <div style={S.p}><span style={S.b}>Дипломатия</span> — международный вес страны. Ниже 15 — поражение через изоляцию. Растёт от дипломатических операций и договорённостей. Падает от эскалации и провокаций. Высокое доверие союзников (Китай, КНДР, Беларусь) частично смягчает удар санкций по экономике.</div>
      ...
      <WikiSection id="kremlin" label={<>ВКЛАДКА «БАШНИ КРЕМЛЯ» И ВОЕННЫЕ ДЕЙСТВИЯ</>}>
        ...
        <div style={S.p}>В каждом месяце доступна <span style={S.b}>одна</span> военная операция. Хотите нанести два удара подряд — сначала используйте перегруппировку: она открывает второй слот в текущем месяце, но блокирует военные (кроме разведки) в следующем.</div>
        <div style={S.p}><span style={S.b}>Сопротивление ВСУ</span> — наступление не проходит бесплатно: каждый раз, когда войска продвигаются, ВСУ контратакуют и отбирают часть территории обратно — на любом направлении, включая Донецк и Луганск, не только на второстепенных участках фронта. Чем ниже боевой дух, готовность и опыт войск — тем сильнее откат. Наступление также стоит потерь боевого духа армии от самих боестолкновений — итоговый прирост территории и изменение духа в конце хода уже учитывают это сопротивление, это не отдельная случайность сверху.</div>
        <div style={{ ...S.p, marginBottom: 0 }}><span style={S.b}>Тайные операции</span> — разведывательные и диверсионные операции (вкладка «Башни Кремля» → «Разведка») несут <span style={S.b}>риск раскрытия</span>. Чем рискованнее операция (ликвидация опаснее дезинформации), тем выше шанс, что она попадёт в новости и ударит по дипломатии и рейтингу. В предпросмотре указа исход тайной операции не раскрывается заранее — вы подписываете указ, не зная наверняка, останется ли он в тени.</div>
      </WikiSection>

      <WikiSection id="ukraine" label="УКРАИНА">
        ...
        <div style={{ ...S.p, marginBottom: 0 }}><span style={S.b}>Ответ на действия Украины</span> — на каждое действие ВСУ доступны три варианта ответа: <span style={S.b}>защититься</span> (дешевле, безопаснее), <span style={S.b}>нанести ответный удар</span> (дороже — 20 инициативы вместо 10 у защиты — и рискованнее: может окупиться, а может ухудшить дипломатический климат, плюс всегда двигает счётчик военной эскалации на +1) или <span style={S.b}>принять ситуацию</span> (бесплатно, но и рейтинг может слегка просесть от бездействия). Исход у защиты и ответного удара вероятностный — под каждой кнопкой видны точные проценты и последствия по каждому исходу, прежде чем вы выберете.</div>
      </WikiSection>

      <WikiSection id="econ" label="ЭКОНОМИЧЕСКИЕ ИНДИКАТОРЫ — ДЕТАЛИ">
        ...
        <div style={{ ...S.p, marginBottom: 0 }}><span style={S.b}>Прозрачность показателей</span> — на экране результата хода каждый показатель разбит по источникам: <span style={S.b}>указ</span> (ваше решение) и <span style={S.b}>события</span> (мировые реакции, кризисы, бюджет). Если экономика упала на 5 при вашем указе на +2 — события дали −7. Это намеренно: мир давит независимо от ваших решений.</div>
      </WikiSection>

      <WikiSection id="policies" label="ВКЛАДКА «ПОЛИТИКИ»">
        <div style={{ ...S.p, marginBottom: 0 }}>Реформы и программы (в т.ч. уже действующие на старте партии — например, повышение НДС до 22% и утилизационного сбора) отображаются во вкладке <span style={S.b}>Политики</span>. Пока политика активна, она может каждый месяц пополнять казну и/или требовать содержания — это видно прямо на карточке («пока действует: казна +X/мес.»). Непопулярные, но доходные меры (вроде НДС) вдобавок вызывают постоянное недовольство — небольшой минус к рейтингу каждый месяц, отдельно от штрафа за отмену. Часть политик имеет срок (реформа/программа с конкретным ходом завершения), часть — бессрочна, пока её явно не отменить. Отмена даёт свои последствия (обычно рост рейтинга/стабильности, но потерю дохода) — они всегда показаны заранее, прежде чем подтвердить отмену.</div>
      </WikiSection>

      <WikiSection id="victory" label="ПУТИ К ПОБЕДЕ И УСЛОВИЯ ПОРАЖЕНИЯ">
        ...
        <div style={S.p}>Партия проверяется на поражение каждый месяц — если один из показателей рушится ниже критической черты, игра заканчивается немедленно:</div>
        <div style={S.p}>
          <span style={S.b}>Рейтинг</span> ниже 30 — государственный переворот.<br/>
          <span style={S.b}>Экономика</span> ниже 30 — экономический коллапс.<br/>
      ...
      <WikiSection id="advisors" label="СОВЕТНИКИ И УКАЗЫ">
        ...
        <div style={{ ...S.p, marginBottom: 0 }}>Хороший собственный указ часто лучше совета: вы лучше знаете ситуацию. Чем конкретнее формулировка — тем точнее игра рассчитает эффект.</div>
      </WikiSection>
    </div>
  );
}

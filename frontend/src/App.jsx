    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div style={{ background: "#14181f", border: "1px solid #3a4156", borderTop: "3px solid #9c8347", borderRadius: 6, maxWidth: 600, width: "100%", maxHeight: "92vh", overflow: "auto", boxShadow: "0 30px 80px rgba(0,0,0,0.8)" }}>
        ...
        <div style={{ padding: "22px 22px 28px" }}>
          ...
          <BriefSection id="decisiontypes" label={<><span style={{ color: "#c0392b" }}>★</span> КАБИНЕТ МИНИСТРОВ И БАШНИ КРЕМЛЯ</>} color="#9c8347">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
              {[
                { label: "🏛 Кабинет министров", desc: "5 министров исполняют ваши указы — у каждого своя область. Кнопка «Распоряжения» на его карточке открывает готовые категории.", color: "#5a9c6a" },
                { label: "★ Башни Кремля", desc: "4 группы элит (силовики, технократы, олигархи, консерваторы) лоббируют свои интересы. Президент — арбитр между ними, не исполнитель их воли.", color: "#c0392b" },
                { label: "⚔️ Военное и разведка", desc: "Военные операции — у министра обороны (от 15 до 80 инициативы). Тайные операции — у главы Совбеза, риск раскрытия растёт с тяжестью.", color: "#9c6347" },
                { label: "🤝 Дипломатия и указы", desc: "Переговоры и мирные инициативы — у главы МИД. Экономические, административные и информационные указы — у остальных министров.", color: "#4a6b8c" },
              ].map(({ label, desc, color }) => (
                <div key={label} style={{ background: "#1a2030", borderRadius: 3, padding: "7px 9px" }}>
                  <div className="mono-font" style={{ fontSize: 9, color, fontWeight: 700, marginBottom: 2 }}>{label}</div>
                  <div className="doc-font" style={{ fontSize: 10.5, color: "#5a6070", lineHeight: 1.3 }}>{desc}</div>
            ...
            <div className="doc-font" style={{ fontSize: 10.5, color: "#5a6070", lineHeight: 1.4, marginTop: 8 }}>
              30 готовых категорий указов, разбросанных по 5 министрам. Откройте карточку министра → «Распоряжения» → категорию — появятся 3 готовые формулировки указа или поле для своего текста. Можно также писать правительству напрямую в свободном поле внизу экрана.
            </div>
          </BriefSection>
          ...
          <BriefSection id="howto" label="📖 КАК ИГРАТЬ" color="#5a6070">
            <div style={{ display: "grid", gap: 10 }}>
              {[
                ...
                ["2", "Откройте «Кабинет министров»", "У каждого министра — кнопка «Распоряжения»: 30 готовых категорий по пяти областям (военное, шпионаж, дипломатия, экономика, информация). Разверните карточку — 3 готовые формулировки указа или своё поле для текста, он сам подставится в форму подписи внизу. А в «Башнях Кремля» элиты лоббируют свои интересы — иногда придётся выбирать, чью сторону принять."],
...
// Отдельный пул фоновых заголовков для состояния "ядерный кризис" (Петя, 2026-07-13: "во время
// ядерного удара новости должны быть совсем другие — сейчас это выглядит нереалистично"). Раньше
// дайджест "Мировые новости" добивал пустые слоты ОБЫЧНЫМ LIVE_HEADLINES (тон мирного нарастания
// напряжённости — курсы валют, санкции, дипломатия) независимо от того, применялось ли ядерное
// оружие в партии — реальный ядерный удар (3 реакции + до 3 мировых ходов, см. turns.js) не всегда
// сразу набирает 6 записей, и добивка мирными заголовками ("биткоин вырос на 12%") рядом с
// новостью о ядерном ударе выглядела абсурдно. Используется, когда в state.newsfeed уже есть
// хотя бы одна запись типа nuclear_reaction (тот же признак isNuclearWorld, что красит шапку/
// таб-бар в NK — см. App).
const NUCLEAR_LIVE_HEADLINES = [
  { src: "Reuters", text: "Применение ядерного оружия подтверждено — ООН созывает экстренное заседание Совбеза" },
  { src: "AP", text: "Мир на грани: аналитики фиксируют резкий рост риска полномасштабной ядерной войны" },
  { src: "BBC", text: "Правительства объявляют военное положение — эвакуация из крупных городов началась" },
  { src: "CNN", text: "Пентагон: стратегические силы США переведены на повышенный уровень боеготовности" },
  { src: "Al Jazeera", text: "Первые данные о радиационном заражении — зона отчуждения расширяется" },
  { src: "France 24", text: "НАТО обсуждает применение статьи 5 — союзники расходятся в оценке порога реагирования" },
  { src: "DW", text: "Мировые биржи обрушились — торги приостановлены на нескольких площадках" },
  { src: "ТАСС", text: "МИД: удар — вынужденный ответ на прямую угрозу существованию государства" },
  { src: "WSJ", text: "Цена золота и нефти взлетела на фоне паники инвесторов после ядерного удара" },
  { src: "Sky News", text: "ВОЗ предупреждает о гуманитарной катастрофе — больницы в зоне поражения переполнены" },
  { src: "Xinhua", text: "Пекин призывает обе стороны к немедленной деэскалации, выражает крайнюю обеспокоенность" },
  { src: "Der Spiegel", text: "Правительства Европы созывают экстренные заседания советов безопасности" },
  { src: "Axios", text: "Источники в Белом доме: рассматриваются все варианты ответа, включая ядерный" },
  { src: "AFP", text: "Волна беженцев из зоны удара — соседние страны закрывают границы" },
  { src: "Bloomberg", text: "Рейтинговые агентства предупреждают о риске глобальной рецессии после удара" },
  { src: "The Times", text: "Разведка союзников фиксирует движение стратегических сил в ответ на удар" },
  { src: "Kyodo", text: "Япония и Южная Корея переводят системы ПРО в режим максимальной готовности" },
  { src: "Le Monde", text: "Папа Римский и религиозные лидеры мира призывают к немедленному прекращению эскалации" },
];
...
function NewsLiveFeed({ state }) {
  ...
  const headlines = useMemo(() => {
    ...
    // Ядерный кризис — добиваем тематически подходящим пулом, не мирными заголовками про курсы
    // валют и санкции (см. NUCLEAR_LIVE_HEADLINES выше).
    const isNuclearWorld = (state?.newsfeed || []).some(n => n.type === "nuclear_reaction");
    const pool = isNuclearWorld ? NUCLEAR_LIVE_HEADLINES : LIVE_HEADLINES;
    const filler = pool.slice(0, NEWS_DIGEST_SIZE - fromGame.length)
...
const SUBSTAT_META = {
  ...
  military: [
    ...
    { key: "veterans",    label: "Опыт войск",  color: "#7a3030", desc: "Четыре года активных боёв дали огромный боевой опыт — крупнейший в Европе со времён ВОВ." },
  ],
  diplomacy: [
...
const KREMLIN_CATEGORIES = {
  ...
  decrees: [
    ...
    { id: "econ_tech", domain: "Экономика", title: "Технологии", desc: "НИОКР, космос, ИИ-суверенитет — небольшие затраты сейчас, рост экономики со временем. Это гражданская и научно-технологическая ставка, не прямой военный бюджет (за ним — к Минобороны).", effects: { military: 1, stability: 1, diplomacy: 1, approval: 1 },
      variants: [
    ...
    { id: "mil_admin_budget", domain: "Военно-административные", title: "Оборонный бюджет", desc: "Прямой контракт с ВПК и войсками — закупки, техника, снабжение армии. НЕ боевая операция (за гражданскими технологиями и НИОКР — к Минфину).", effects: { military: 1, stability: 1 },
      variants: [
    ...
    { id: "pol_social", domain: "Экономика", title: "Социальная программа", desc: "Маткапитал, пенсии, здравоохранение, демография — двигает одобрение народа.", effects: { stability: 1, approval: 1 },
      variants: [
...
const MINISTER_DOMAINS = {
  ...
  press: [
    { id: "info", label: "📰 Информационная политика", mode: null, cards: KREMLIN_CATEGORIES.decrees.filter(c => ["pol_liberalization", "pol_propaganda"].includes(c.id)) },
  ],
};

...
function computeStrategicMove(stats, turn) {
  ...
  // Шаг считается выполненным не по номеру хода (в режиме мульти-действий turn
  // не меняется, пока не закончится месяц), а по факту: буфер уже создан.
  const step1Done = eco >= 50;
  if (turn <= 1 && !step1Done) {
    return {
      priority: 9, icon: "🏛", strategic: true,
      title: "Шаг 1 из 4: казна вперёд оружия",
  ...
  if (mil < 68) {
    return {
      priority: 9, icon: "🪖", strategic: true,
      title: "Шаг 2 из 4: укрепить армию",
  ...
  if (init < 55) {
    return {
      priority: 9, icon: "⚙", strategic: true,
      title: "Шаг 3 из 4: перегруппировка — копим инициативу",
  ...
  return {
    ...
    title: "Шаг 4 из 4: наступать — есть инициатива и казна",
